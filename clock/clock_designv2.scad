...
module minute_hand(){
    // Minute hand - longer with 3mm hole at base and circular boss
    difference() {
        union() {
            // Thinner main hand
            color("gray") translate([0,0,5])
                cube([60,4,1], center=true);
            // Circular boss around the hole
            translate([24,0,4.5])
                cylinder(d=10, h=1, $fn=30);
        }
        // 3mm hole through the boss
        translate([24,0,4])
...
module hour_hand(){
    // Hour hand - shorter with 6mm hole at base and circular boss
    difference() {
        union() {
            // Thinner main hand
            color("gray") translate([0,0,4])
                cube([40,4.5,1], center=true);
            // Circular boss around the hole
            translate([14,0,3.5])
                cylinder(d=12, h=1, $fn=35);
        }
        // 6mm hole through the boss
        translate([14,0,3])
            cylinder(d=6, h=2, $fn=35);
    }
}
